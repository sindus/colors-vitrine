...
export function IndemodablesSection({ data }: Props) {
  const overline = data?.overline ?? "— Vos préférés —";
  const title = data?.title ?? "Les indémodables";
  const products = data?.products?.length ? data.products : PRODUCTS.slice(4, 8);
